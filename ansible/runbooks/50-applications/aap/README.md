...
- `00-podman-storage-bootstrap.yml`: configure and verify rootless Podman
  ...
  verify the transient runroot below `/run/user` before the
  execution-environment image is available; this bootstrap intentionally uses
  only `ansible.builtin` and the dedicated `ansible-bootstrap.cfg`, so it does
  not require the project-wide Vault password file.
